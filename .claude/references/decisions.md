...
| 부분 실패 표현 | 공급사 단위. 응답 `failures[]`에 (supplier, type, code) 한 건. 50개 묶음 중 일부만 실패해도 그 공급사를 실패로 표시하되 성공한 묶음의 상품은 그대로 응답. 검색은 공급사 N개 fan-out이라 실패 단위가 요청 전체가 아니라 공급사 하나다. 예외를 그대로 올리면 성공한 공급사의 결과까지 버려지고, "한 공급사가 늦어도 나머지로 5초 안에 응답"이라는 예산 설계도 무의미해진다(HTTP 상태는 하나뿐이라 일부 성공을 표현할 수 없고, 207은 WebDAV용이라 쓰지 않는다). 값 타입은 `supplier/vo/SupplierFailure`이고 `SupplierCallException.toFailure()`가 만든다: 예외가 이미 같은 세 필드를 들고 있으므로 도메인에서 같은 모양을 다시 선언하지 않는다. **공급사가 전부 실패하면 예외를 올려 502**(`ApiControllerAdvice`). `rooms: []` + `failures`로 200을 주면 "검색 성공, 결과 0건"과 구별되지 않기 때문. `AppException(SUPPLIER_ALL_FAILED, failures)`로 올려 실패한 공급사 목록(`List<SupplierFailure>`)을 `ErrorMessage.data`에 담는다(`E2006`, 502, WARN). 첫 실패를 다시 던지는 방식은 채택하지 않음: `SupplierCallException` 생성자가 `data`를 `supplier type code` 한 건으로 고정해서 공급사 하나만 실린다. `data`에 구조체를 담는 건 검증 실패가 `{필드: 사유}` Map을 담는 것과 같은 방식이다 |
...
| 공급사 예외 계층 | `SupplierCallException`은 `AppException`을 상속한다(별도 계층으로 두지 않음). `SupplierFailureType`의 각 값이 대응 `ErrorType`을 들고 있고(`SUPPLIER_BAD_REQUEST`~`SUPPLIER_MALFORMED`, `E2000`~`E2005`, 전부 502·WARN. `SUPPLIER_ALL_FAILED`(`E2006`)는 `SupplierFailureType`에 대응 값이 없고 검색 Service가 직접 던진다), 생성자가 `type.getErrorType()`을 `super`로 넘긴다. `data`에는 `supplier type code`를 담아 `ApiControllerAdvice`·로그에서 원본 코드를 본다. `AppException`에 `(ErrorType, data, cause)` 생성자를 추가했다. VO 검증 실패는 기존대로 `AppException(INVALID_SUPPLIER_STAY)`라서 `StaySyncService`는 `SupplierCallException`(호출 실패) → `AppException`(항목 검증) 순으로 잡는다 |
...
| 검색 응답 | `rooms[]`(stayId, stayName, roomTypeId, roomTypeName, maxOccupancy, availableRooms, supplier, breakfastIncluded, currency, totalPrice) + `failures[]`. 예약 불가는 `availableRooms=0`으로 노출하고 빼지 않는다. 원소가 숙소가 아니라 객실 한 건이므로 필드명은 `stays`가 아니라 `rooms`(한 숙소의 객실이 여러 건이면 같은 숙소가 여러 번 나온다). 파생 필드 `available`은 두지 않는다: `availableRooms > 0`과 같은 값을 두 번 주고, 둘이 어긋나면 어느 쪽이 맞는지 계약에 없다 |
| 검색 요청 검증 | `GET /api/v1/stays/search`는 `StaySearchRequest`(record, `@Valid @ModelAttribute` 생성자 바인딩) → `toPeriod()`·`toGuests()` → `StaySearchService.search(StayPeriod, Guests)`. 입력을 한 번 더 감싸는 Command record는 두지 않는다(아래 검색 계층 구성). **검증은 두 곳**: 필드 하나짜리 제약은 Request의 Bean Validation(`@NotNull` 날짜·성인, `@Min(1)` 성인, `@Min(0)` 아동)이 HTTP 계약으로 잡아 E400 + `{필드: 사유}`를 한 번에 돌려주고 OpenAPI 스키마에 `minimum`·`required`로 드러난다. 필드 간 규칙과 불변식은 VO 생성자: `StayPeriod`는 null·`checkOut ≤ checkIn`이면 `INVALID_STAY_PERIOD`(E1001), 체크인이 오늘보다 앞이면 `PAST_CHECK_IN`(E1002), 30박 초과면 `STAY_PERIOD_TOO_LONG`(E1003). `Guests`는 성인 null·1 미만, 아동 0 미만이면 `INVALID_GUESTS`(E1004). 중복으로 보지 않는 이유는 Request가 HTTP 계약을, VO가 객체 불변식을 지키기 때문(HTTP 밖 경로 보호). 필드 간 규칙을 Bean Validation으로 옮기려면 클래스 레벨 커스텀 제약이 필요해 "별도 Validator 금지"와 충돌하므로 옮기지 않는다. 아동 null은 VO 생성자가 0으로 채운다. 오늘 판정은 서버 기본 시간대의 `LocalDate.now()` |
| 검증 실패 응답 | `MethodArgumentNotValidException`(`@ModelAttribute` + record도 이 타입)을 `ApiControllerAdvice`가 잡아 `INVALID_ACCESS_PATH`(E400) `data`에 `{필드: 사유}`를 담는다. 같은 필드에 제약이 여러 개면 첫 사유만. 타입 변환 실패(`isBindingFailure`)는 기본 메시지에 내부 타입·어노테이션이 들어 있어 "형식이 올바르지 않습니다"로 바꾼다. 로그는 `[요청 검증 : 실패]: 필드=사유 \| 필드=사유` | 오늘 판정은 서버 기본 시간대의 `LocalDate.now()` |
| 검색 계층 구성 | `StayController.search` → `StaySearchService.search(period, guests)` → `StaySearchResult(rooms, failures)` VO → `ApiResponse.success(result)`. `SearchedRoom`·`SupplierFailure`·`StaySearchResult`는 `stay/vo`. **입력 Command record도, 출력 Response record도 없음**: 필드 2개에 동작이 없고 호출부가 한 곳이라 파라미터 목록의 별칭에 지나지 않았고, 출력만 `vo`에 있고 입력만 `business`에 있는 비대칭도 생겼다. VO를 그대로 파라미터로 넘긴다. 정렬·필터처럼 입력이 늘어 시그니처가 길어지면 그때 `stay/vo`에 조건 VO를 만든다. 출력도 같은 이유로 `controller/response`를 비웠다: `SearchedRoomResponse`·`SupplierFailureResponse`·`StaySearchResponse` 세 record가 대응 VO와 필드가 완전히 같고 `from`은 값을 그대로 옮기기만 해서, 계층 경계라는 이름으로 코드만 늘리고 있었다. VO를 그대로 `ApiResponse.success()`에 넣는다. 대가로 VO 필드 변경이 곧 API 계약 변경이 되므로, 응답 모양이 VO와 갈라지는 일(내부 enum을 외부 문자열로 번역, 파생 필드, `@Schema` 문서화)이 생기면 그때 `controller/response`를 다시 만든다 |
...
| 검색 결과 단위 이름 | 검색 결과 한 건 = 기간·인원 조건으로 팔 수 있는 객실 1건(숙소·객실 정보 + 총액 + 잔여 수). 도메인 VO `SearchedRoom`, 응답 필드 `rooms[]`, 공급사 입력 VO `SupplierRoom`, 공급사 호출 `searchRooms()`로 room 계열 한 단어로 통일한다. `Offer`는 숙소 단위인지 객실 단위인지 이름에서 안 읽혀 쓰지 않는다. `AvailableRoom`·`BookableRoom` 계열은 `availableRooms=0`도 응답에 포함하기로 한 결정과 이름이 어긋나 쓰지 않는다. `Room`은 `RoomType` 엔티티와 헷갈려 쓰지 않는다. 공급사 호출은 하루 한 번 정적 마스터를 받는 `fetchStays()`와 매 요청 실시간으로 받는 `searchRooms()`를 `fetch`/`search`로 구분한다 |
